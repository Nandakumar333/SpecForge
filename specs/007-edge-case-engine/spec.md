...
- **FR-006**: Each edge case MUST include a YAML frontmatter block with machine-parseable fields: `id`, `category`, `severity`, `affected_services`, `handling_strategy`, `test_suggestion`. The `trigger` field is rendered in the human-readable markdown section below the YAML block, not inside the YAML frontmatter
...
- **FR-016**: Severity levels MUST be one of: `critical`, `high`, `medium`, `low` — assigned via a deterministic severity matrix:
  - **Microservice inter-service**: required+sync-rest/sync-grpc → `critical`; required+async-event → `high`; optional+sync → `high`; optional+async → `medium`
...
- **FR-018**: Edge case count per service MUST follow this budget: 6 base cases (1 per standard category) + 2 per external dependency + 1 per event role (each unique event where the service is producer counts as 1; each unique event where the service is consumer counts as 1) + 2 per additional feature beyond the first (feature-interaction cases). The budget formula applies to ALL architectures — a monolith with 3 features produces 6 + 0 + 0 + 2×(3-1) = 10 cases. Data ownership cases from BoundaryAnalyzer count within the 2-per-dependency allocation (not additive). Feature-interaction cases cover scenarios where two features within the same service have conflicting resource needs (e.g., account creation and transaction processing competing for write locks). Total MUST NOT exceed 30 per service. When truncation is needed, cases are prioritized by severity (critical > high > medium > low), then by category priority order
...
- **SC-001**: Edge cases generated for a service with N dependencies, E event roles (producer + consumer), and F features include exactly 6 + 2N + E + 2×max(0, F-1) edge cases, capped at 30 total per service
