...
- Q: How are tech-stack variants organized on disk — subdirectories or name suffixes? → A: **Subdirectories per template**. Each template that supports stack variants gets a subdirectory: `prompts/backend.md.j2` (generic), `prompts/backend/dotnet.md.j2` (stack-specific), `prompts/backend/nodejs.md.j2`. The generic file sits alongside the subdirectory. User overrides follow the same structure in `.specforge/templates/`.
...
- **FR-008**: The TemplateRegistry MUST resolve template lookups using the following precedence order: (1) user override, (2) stack-specific variant, (3) generic built-in.
...
- **FR-013**: The engine MUST validate rendered output for unresolved placeholder markers. Any remaining placeholders MUST be reported with their line number and placeholder name.
...
- **FR-017**: Each Agent Instruction Prompt template MUST support tech-stack-specific variants organized as subdirectories on disk. The generic template sits at the template type level (e.g., `prompts/backend.md.j2`) while stack variants reside in a same-named subdirectory (e.g., `prompts/backend/dotnet.md.j2`, `prompts/backend/nodejs.md.j2`). The generic variant serves as the fallback for stacks without a dedicated variant.
- **FR-018**: The engine MUST support **7 Per-Feature** templates: spec, research, data-model, plan, checklist, edge-cases, and tasks — each with contextually appropriate placeholder variables and conditional sections.
- **FR-019**: Per-Feature templates MUST include smart placeholders that provide contextual hints (e.g., a placeholder for "acceptance criteria" in the spec template should include guidance on how to write good acceptance criteria).

#### User Overrides

- **FR-020**: Custom templates placed in the project's `.specforge/templates/` directory MUST automatically override built-in templates with the same relative path — no configuration changes required. User overrides for stack variants follow the same subdirectory structure (e.g., `.specforge/templates/prompts/backend/dotnet.md.j2`).
...
- **Stack Variant**: A specialized version of a template tailored to a specific technology stack. Organized on disk as a subdirectory alongside the generic template (e.g., `prompts/backend/dotnet.md.j2`). Falls back to the generic version when no variant exists for the requested stack.
...
- Stack variants are organized as subdirectories on disk: `prompts/backend.md.j2` (generic) alongside `prompts/backend/dotnet.md.j2` (stack-specific). This was chosen over name suffixes for clearer file tree navigation, avoidance of name collisions, and intuitive override paths.
