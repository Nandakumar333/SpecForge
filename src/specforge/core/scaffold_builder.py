...
def _build_files(config: ProjectConfig) -> list[ScaffoldFile]:
    """Build the ordered list of scaffold files."""
    ...
    for name in PROMPT_FILES:
        files.append(ScaffoldFile(
            relative_path=Path(f".specforge/prompts/{name}.md"),
            template_name=f"prompts/{name}.md.j2",
            context=ctx,
        ))
    ...
    for name in FEATURE_TEMPLATES:
        files.append(ScaffoldFile(
            relative_path=Path(f".specforge/templates/features/{name}.md"),
            template_name=f"features/{name}.md.j2",
            context=ctx,
        ))
